...
Stmt* make_stmt_arithmetic(Value left, ArithmeticOperation op, Value right);
...
%type <stmt> stmt if_stmt write read while expr_value logical_expr arithmetic_expr 
...
Stmt* make_stmt_factor_var(string variable) {
  Stmt* s = (Stmt*) malloc(sizeof(Stmt));

  int si = get_symbol_index(variable);
  if (si == -1) {
  ...
  switch (symb->type) {
    case TYPE_INT: {
      s->factor_stmt.value.type = TYPE_INT;
      s->factor_stmt.value.i = symb->i;
      break;
    }
    case TYPE_FLOAT: {
      s->factor_stmt.value.type = TYPE_FLOAT;
      s->factor_stmt.value.f = symb->f;
      break;
    }
    case TYPE_BOOL: {
      s->factor_stmt.value.type = TYPE_BOOL;
      s->factor_stmt.value.b = symb->b;
      break;
    }
    case TYPE_CHAR: {
      s->factor_stmt.value.type = TYPE_CHAR;
      s->factor_stmt.value.c = symb->c;
      break;
    }
    default:
      break;
...
Stmt* make_stmt_factor(any val, bool minus_unary) {
  Stmt* s = (Stmt*) malloc(sizeof(Stmt));
  s->factor_stmt.value.type = val.type;

  if (val.type == TYPE_INT) {
    s->factor_stmt.value.i = minus_unary ? -val.i : val.i;
  } else {
    s->factor_stmt.value.f = minus_unary ? -val.f : val.f;
  }

  return s;
}

int evaluate_arithmetic_expr(Stmt* s) {
...
int evaluate_logical_expr(Stmt* s) {
  switch (s->logical_stmt.op) {
    case OP_GT:
      return evaluate_arithmetic_expr(s->logical_stmt.left) > evaluate_arithmetic_expr(s->logical_stmt.right);
      break;
    case OP_GTE:
      return evaluate_arithmetic_expr(s->logical_stmt.left) >= evaluate_arithmetic_expr(s->logical_stmt.right);
      break;
    ...
    case OP_DIFF:
      ...
      break;
    case OP_AND:
      return evaluate_logical_expr(s->logical_stmt.left) && evaluate_logical_expr(s->logical_stmt.right);
      break;
    case OP_OR:
      return evaluate_logical_expr(s->logical_stmt.left) || evaluate_logical_expr(s->logical_stmt.right);
      break;
    case OP_NOT:
      return !evaluate_logical_expr(s->logical_stmt.right);
      break;
  }
}
