...
pub unsafe extern "C" fn HttpGet(url_ptr: *const c_char) -> HttpResponse {
    ...
    println!("HttpGet: {}", url);
    let resp =
        get_runtime().block_on(async move { reqwest::get(url).await.expect("Request failed") });
...
pub extern "C" fn ToJson(resp: HttpResponse) -> *const c_char {
    ...
    let json_text = get_runtime()
        .block_on(async move { response.text().await.unwrap_or_else(|_| "{}".to_string()) });
...
pub unsafe extern "C" fn HttpRoute(
    ...
    let method_str = CStr::from_ptr(method)
        .to_str()
        .unwrap()
        .to_string()
        .to_lowercase();

    println!("Route registered: {} [{}]", path_str, method_str);
...
pub unsafe extern "C" fn HttpServe(listener: HttpListener, router: HttpRouter) {
    ...
    println!("Server entering blocking loop...");
    get_runtime().block_on(async move {
        axum::serve(axum_listener, axum_router)
            .await
