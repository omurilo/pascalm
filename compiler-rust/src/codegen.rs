...
impl<'ctx> CodeGen<'ctx> {
                    ...
                    for (i, id) in ids.iter().enumerate() {
                        ...
                        self.variables[0].insert(
                            ...
                            (
                                ...
                                val.get_type().as_basic_type_enum(),
                                TypeExpr::Simple(t_decl.name.clone()),
                            ),
                        );
                    }
                }
        ...
        if let Some(units) = uses {
            for unit_name in units {
                ...
                if let Some(interface) = self.external_interfaces.get(&unit_lower).cloned() {
                    // First pass: Process Types
                    for (name, kind) in &interface {
                        if let SymbolKind::Type { type_expr } = kind {
                            let llvm_type = self.resolve_type(type_expr)?;
                            self.types.insert(name.clone(), (llvm_type, type_expr.clone()));
                        }
                    }
                        ...
                        match kind {
                            SymbolKind::Variable { type_expr } => {
                                ...
                                if self.module.get_global(&name).is_none() {
                                    ...
                                    self.variables[0].insert(
                                        name.clone(),
                                        (global.as_pointer_value(), llvm_type, type_expr.clone()),
                                    );
                                }
                            }
                                    ...
                                    for p in params {
                                        match p {
                                            Param::Variable { type_name, ids, .. } => {
                                                let pt = self.resolve_type(&TypeExpr::Simple(
                                                    type_name.clone(),
                                                ))?;
                                                for _ in ids {
                            ...
                            SymbolKind::Function {
                                ...
                                if self.module.get_function(&name).is_none() {
                                    ...
                                    let rt_llvm =
                                        self.resolve_type(&TypeExpr::Simple(return_type.clone()))?;
    ...
    fn gen_block(&mut self, block: &Block, function: FunctionValue<'ctx>) -> Result<(), String> {
        // We no longer pre-create label blocks to avoid empty blocks in LLVM IR
        // if they are declared but never used.
        ...
        if let Some(constants) = &block.constants {
            for c in constants {
                ...
                ); // Defaulting to integer for constants
            }
        }
                ...
                for id in &v_decl.ids {
                    if self.variables.len() == 1 {
                        ...
                        self.variables
                            .last_mut()
                            .unwrap()
                            .insert(id.clone(), (ptr, llvm_type, v_decl.type_expr.clone()));
                    }
                }
            }
        }
    ...
    fn gen_proc_func(&mut self, decl: &ProcFuncDecl) -> Result<(), String> {
        let (name, params, return_type, block_or_forward) = match decl {
            ProcFuncDecl::Procedure {
                ...
                params,
                block_or_forward,
            } => (name, params, None, block_or_forward),
            ProcFuncDecl::Function {
                name,
                params,
                return_type,
                block_or_forward,
            } => (name, params, Some(return_type), block_or_forward),
        };
        ...
        if let BlockOrForward::Block(b) = block_or_forward {
                        ...
                        Param::Variable { type_name, ids, .. } => {
                            ...
                            for id in ids {
                                ...
                                self.variables
                                    .last_mut()
                                    .unwrap()
                                    .insert(id.clone(), (ptr, pt, pt_expr.clone()));
                                arg_idx += 1;
                            }
                        }
                        Param::Procedure { id, .. } => {
                            ...
                            self.variables.last_mut().unwrap().insert(
                                id.clone(),
                                (ptr, pt.into(), TypeExpr::Simple("procedure".to_string())),
                            );
                            arg_idx += 1;
                        }
                        Param::Function {
                            ...
                            self.variables.last_mut().unwrap().insert(
                                ...
                                (
                                    ptr,
                                    pt.into(),
                                    TypeExpr::Simple(format!("function:{}", return_type)),
                                ),
                            );
                            arg_idx += 1;
                        }
            ...
            if let Some(rt) = return_type {
                ...
                self.variables
                    .last_mut()
                    .unwrap()
                    .insert(name.clone(), (ret_ptr, rt_llvm, rt_expr));
            }
    ...
    fn resolve_type(
        ...
        match te {
            ...
            TypeExpr::Record {
                ...
                self.record_fields
                    .insert(format!("{:?}", llvm_type), field_map);
                Ok(llvm_type)
            }
            TypeExpr::Array {
                indices,
                ...
                for idx_te in indices {
                    match idx_te {
                        ...
                        TypeExpr::Simple(name) if name == "boolean" => {
                            total_size *= 2;
                        }
                        TypeExpr::Simple(name) if name == "char" => {
                            total_size *= 256;
                        }
                        _ => {
                            total_size = 100;
            ...
            TypeExpr::Pointer(_) | TypeExpr::File(_) | TypeExpr::Procedure { .. } | TypeExpr::Function { .. } => Ok(self
                .context
                .ptr_type(inkwell::AddressSpace::default())
                .as_basic_type_enum()),
            TypeExpr::Set(_) => Ok(self.context.custom_width_int_type(256).as_basic_type_enum()),
    ...
    fn gen_stmt(&mut self, stmt: &Stmt, function: FunctionValue<'ctx>) -> Result<(), String> {
        match stmt {
            Stmt::Labeled(l, s) => {
                let target_bb = self.get_or_create_label_bb(function, *l);

                // Only build fallthrough if current block is not terminated
                if self
                    .builder
                    .get_insert_block()
                    .unwrap()
            ...
            Stmt::Repeat { body, until } => {
                ...
                self.builder
                    .build_conditional_branch(not_cond, loop_bb, after_bb)
                    .map_err(|e| e.to_string())?;
            ...
            Stmt::For {
                ...
                let current_val = self
                    .builder
                    .build_load(var_type, var_ptr, "current_for_val")
                    .unwrap()
                    .into_int_value();
            ...
            Stmt::Case {
                ...
                for item in items {
                    ...
                    for label in &item.labels {
                        match label {
                            ...
                            _ => {
                                let mut label_val = self.gen_expr(label)?.into_int_value();
                                // Switch requires a constant integer. If gen_expr returned a load instruction
                                // (e.g. from an enum global constant), try to extract its constant initializer.
                                if !label_val.is_constant_int() {
                                    if let Expr::Variable(v) = label {
                                        if let Variable::Id(id) = &**v {
                                            if let Some(global) = self.module.get_global(id) {
                                                if global.is_constant() {
                                                    if let Some(init) = global.get_initializer() {
                                                        label_val = init.into_int_value();
                                                    }
                                                }
                                            }
                                        }
                ...
                self.builder
                    .build_switch(case_val, else_bb, &cases)
                    .unwrap();
            ...
            Stmt::ProcedureCall { name, args } => {
                if name == "write" || name == "writeln" {
                    ...
                    if let Some(args) = args {
                        for arg in args {
                            ...
                            self.builder
                                .build_call(
                                    printf,
                                    &[fmt_str.as_pointer_value().into(), val_to_print.into()],
                                    "printf",
                                )
                                .unwrap();
                        }
                    ...
                    if name == "writeln" {
                        ...
                        self.builder
                            .build_call(printf, &[nl.as_pointer_value().into()], "printf")
                            .unwrap();
                    }
                    ...
                    if let Some(args) = args {
                        for arg in args {
                            ...
                            self.builder
                                .build_call(
                                    scanf,
                                    &[fmt_str.as_pointer_value().into(), ptr.into()],
                                    "scanf",
                                )
                                .unwrap();
                        }
                    ...
                    if let Some(function_to_call) = self.module.get_function(real_name) {
                        self.builder
                            .build_call(function_to_call, &llvm_args, real_name)
                            .unwrap();
                    } else {
                        // Indirect call?
                        let (ptr, llvm_type, te) = self
                            .get_variable(name)
                            .ok_or_else(|| format!("Procedure '{}' not found", name))?;
                        let fn_ptr = self
                            .builder
                            .build_load(*llvm_type, *ptr, "fnptr")
                            .unwrap()
                            .into_pointer_value();
                        ...
                        let ret_type = match te {
                            TypeExpr::Simple(n) if n.starts_with("function:") => {
                                ...
                                Some(
                                    self.resolve_type(&TypeExpr::Simple(rt_name.to_string()))
                                        .unwrap(),
                                )
                            }
                            _ => None,
                        ...
                        self.builder
                            .build_indirect_call(fn_type, fn_ptr, &llvm_args, name)
                            .unwrap();
                    }
                }
            }
                    ...
                    } = resolved_te
                    {
                        if let Some(field_indices) = self.record_fields.get(&type_key) {
                            for (field_name, &index) in field_indices {
                                let member_ptr = self
                                    .builder
                                    .build_struct_gep(struct_type, ptr, index, field_name)
                                    .unwrap();
                                let member_type =
                                    struct_type.get_field_type_at_index(index).unwrap();
                                let field_te = self
                                    .find_field_te(&fields, variant_part.as_ref(), field_name)
                                    .unwrap();
                                self.variables.last_mut().unwrap().insert(
                                    field_name.clone(),
                                    (member_ptr, member_type, field_te),
                                );
                            }
                        }
                    }
            ...
            Stmt::Empty => {
                // LLVM doesn't like empty basic blocks.
                // We could insert a 'nop' using a dummy bitwise operation.
                let zero = self.context.i64_type().const_zero();
                self.builder.build_or(zero, zero, "nop").unwrap();
            }
            _ => {}
        }
        Ok(())
        ...
        match expr {
            ...
            Expr::Variable(v) => {
                match self.gen_variable_ptr_with_type(&**v) {
                    Ok((ptr, llvm_type, _)) => Ok(self
                        .builder
                        .build_load(llvm_type, ptr, "loadtmp")
                        .unwrap()
                        .into()),
                    Err(e) => {
            ...
            Expr::Binary { op, left, right } => {
                ...
                match op {
                    ...
                    BinOp::Sub => {
                        if lhs.is_int_value() && rhs.is_int_value() {
                            ...
                            if lw == 256 {
                                let not_rhs = self
                                    .builder
                                    .build_not(rhs.into_int_value(), "notrhs")
                                    .unwrap();
                                Ok(self
                                    .builder
                                    .build_and(lhs.into_int_value(), not_rhs, "setdiff")
                                    .unwrap()
                                    .into())
                            } else {
                                Ok(self
                    ...
                    BinOp::In => {
                        ...
                        let bit = self
                            .builder
                            .build_int_z_extend(element, set_type, "bitext")
                            .unwrap();
                        let mask = self
                            .builder
                            .build_left_shift(set_type.const_int(1, false), bit, "mask")
                            .unwrap();
                ...
                for el in elements {
                    match el {
                        Element::Single(e) => {
                            ...
                            let bit = self
                                .builder
                                .build_int_z_extend(val, set_type, "bitext")
                                .unwrap();
                            let mask = self
                                .builder
                                .build_left_shift(set_type.const_int(1, false), bit, "mask")
                        ...
                        Element::Range(start, end) => {
                            ...
                            let s_ext = self
                                .builder
                                .build_int_z_extend(s, set_type, "sext")
                                .unwrap();
                            let e_ext = self
                                .builder
                                .build_int_z_extend(e, set_type, "eext")
                                .unwrap();
                            ...
                            let mask_s = self
                                .builder
                                .build_left_shift(all_ones, s_ext, "masks")
                                .unwrap();

                            let max_bits = set_type.const_int(255, false);
                            let shift_r = self
                                .builder
                                .build_int_sub(max_bits, e_ext, "shiftr")
                                .unwrap();
                            let mask_e = self
                                .builder
                                .build_right_shift(all_ones, shift_r, false, "maske")
                                .unwrap();

                            let range_mask =
                                self.builder.build_and(mask_s, mask_e, "rangemask").unwrap();
            ...
            Expr::FunctionCall { name, args } => {
                if name == "Chr" {
                    ...
                    return Ok(self
                        .builder
                        .build_int_truncate(arg, self.context.i8_type(), "chr")
                        .unwrap()
                        .into());
                }
                if name == "Ord" {
                    ...
                    return Ok(self
                        .builder
                        .build_int_z_extend(arg, self.context.i64_type(), "ord")
                        .unwrap()
                        .into());
                }
    ...
    > {
        match var {
            Variable::Id(id) => self
                .get_variable(id)
                .cloned()
                .ok_or_else(|| format!("Variable '{}' not found in codegen", id)),
            Variable::MemberAccess { record, field } => {
                ...
                if !llvm_type.is_struct_type() {
                    return Err("Member access target is not a record".to_string());
                }
                let struct_type = llvm_type.into_struct_type();
                let type_key = format!("{:?}", llvm_type);
                let index = *self
                ...
                {
                    let field_te = self
                        .find_field_te(&fields, variant_part.as_ref(), field)
            ...
            Variable::ArrayAccess { array, indices } => {
                ...
                if !llvm_type.is_array_type() {
                    return Err("Array access target is not an array".to_string());
                }
                ...
                let element_ptr = unsafe {
                    self.builder
                        .build_gep(array_type, ptr, &[zero, index], "arraygep")
                        .unwrap()
                };
            ...
            Variable::PointerDeref(p) => {
                ...
                let p_te = match &**p {
                    ...
                    _ => {
                        return Err(
                            "Pointer deref only supported on named variables for now".to_string()
                        )
                    }
                };
    ...
    fn find_field_te(
        ...
        for f in fields {
            if f.ids.contains(&field.to_string()) {
                return Some(f.type_expr.clone());
            }
        }
        if let Some(vp) = variant_part {
            if let Some(tag) = &vp.tag_field {
                if tag == field {
                    return Some(TypeExpr::Simple(vp.tag_type.clone()));
                }
            }
            for variant in &vp.variants {
                if let Some(t) =
                    self.find_field_te(&variant.fields, variant.nested_variant.as_deref(), field)
                {
                    return Some(t);
                }
            }
